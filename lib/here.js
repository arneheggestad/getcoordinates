	...
	parseResponse: function (response, callback) {
		...
		parser.parseString(response, function (err, parsedResponse) {
			if (!parsedResponse) {
				...
				var pointsArray = [];
				var location = parsedResponse.Search.Response[0].View[0].Result[0].Location;
				for (var i = 0; i < location.length; i++ ) {
					var temp = {};
					temp.lat = parseFloat(location[i].DisplayPosition[0].Latitude[0]);
					temp.lon = parseFloat(location[i].DisplayPosition[0].Longitude[0]);
